...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Remove Basic/Premium/Economy/Prudent sections from dropdowns site-wide
    try {
        const sectionsToRemove = new Set(['Basic Services', 'Premium Services', 'Economy Services', 'Prudent Services']);
        document.querySelectorAll('.dropdown-section h4').forEach(h4 => {
            const title = (h4.textContent || '').trim();
            if (sectionsToRemove.has(title)) {
                const section = h4.closest('.dropdown-section');
                if (section) {
                    section.remove();
                }
            }
        ...
        console.warn('Dropdown cleanup failed:', e);
    }
    
    // Remove Cart and Account from navbar
    try {
        document.querySelectorAll('.cart-btn, .account-btn').forEach(el => el.remove());
    } catch (e) {
